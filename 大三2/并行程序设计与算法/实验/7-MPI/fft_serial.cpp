...
//
      ...
      {
        sgn = + 1.0;
        //x：输入数据，变换后的数据会覆盖原内容 y：存储变换结果的数组 w：正弦和余弦表。
        cfft2 ( n, x, y, w, sgn );  // 正向FFT
        sgn = - 1.0;
        cfft2 ( n, y, x, w, sgn );  // 反向FFT
// 
...
//
{
  ...
  for ( j = 0; j < lj; j++ )// lj 递减如4，2，1
  {
    ...
    // 每一轮的操作一样，只有wjw不一样，迭代到后面k越来越大，不同wjw的个数也就是lj越来越小
    for ( k = 0; k < mj; k++ )
    {
